...
def dot1q_vlan_id(hostname, username, password, vlan, description):
    ...
    commands = [
        'config',
        f'dot1q vlan {vlan} name {description} interface ten-gigabit-ethernet-1/1/1 ; top',     # noqa E501
        f'commit and-quit label "dot1q" comment "add vlan {vlan} via netmiko"',                 # noqa E501
        'show dot1q vlan',
    ]

    try:
        ssh = ConnectHandler(**device)
        print(f'[green]Conectado ao Switch {ssh.find_prompt()} - {hostname} ✅[/green]')        # noqa E501
